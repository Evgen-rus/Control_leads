...
- ✅ **Создание лидов в Битрикс24** с автоматическим заполнением полей
...
3. **`bitrix24_upload.py`** - Модуль создания лидов в Битрикс24
4. **`sync_and_notify.py`** - Объединённый скрипт для синхронизации, уведомлений и создания лидов
5. **`scheduler.py`** - Планировщик для автоматического запуска
6. **`.env`** - Конфигурационный файл с настройками
...
1. **Создание вебхука:**
   ...
   - Перейдите в раздел "Приложения" → "Другое" → "Входящий вебхук"
   - Создайте новый вебхук с правами на CRM
   - Скопируйте URL вебхука

2. **Настройка полей лида:**
   - Убедитесь, что в CRM настроены необходимые поля для лидов
   - Проверьте ID ответственного пользователя
   - Настройте источники лидов (ЛидгенБюро)
...
# Битрикс24
BITRIX_WEBHOOK_URL=https://your-domain.bitrix24.ru/rest/your-webhook-url
```
...
python sync_and_notify.py        # Синхронизация + уведомления + Битрикс24
python scheduler.py              # Планировщик
python notifier.py              # Только уведомления (тест)
python bitrix24_upload.py       # Только создание лидов в Битрикс24 (тест)
```
...
├── bitrix24_upload.py         # Модуль создания лидов в Битрикс24
...
**Основные функции:**
- `BitrixLeadUploader` - Класс для работы с API Битрикс24
- `create_lead()` - Создание лида в Битрикс24
- `process_new_rows()` - Обработка массива новых строк
- `upload_leads_to_bitrix()` - Основная функция отправки лидов

**Особенности:**
- Автоматическое заполнение всех полей лида
- Настройка ответственного пользователя (Михаил)
- Источник лидов (ЛидгенБюро)
- UTM-метки для аналитики
- Подробное логирование операций
- Статистика создания лидов
...
```bash
systemctl restart control-leads.service
```
